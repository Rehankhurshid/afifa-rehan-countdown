...
class MSG91Service {
  ...
  async sendSimpleMessage(phoneNumbers, message) {
    ...
    // MSG91 sendhttp.php expects URL encoded parameters
    ...
    console.log("🔗 MSG91 Request Path:", path.substring(0, 100) + "...");
      ...
      const req = https.request(options, (res) => {
        ...
        res.on("end", () => {
          ...
          try {
            ...
            // Check if response is a hex-encoded message ID (successful send)
            // Message IDs are typically hex-encoded strings
            if (/^[a-fA-F0-9]+$/.test(response.trim())) {
              resolve({
                success: true,
