...
async function testMSG91Service() {
  ...
  console.log(
    `Auth Key: ${weddingConfig.messaging.msg91.authKey ? "Found" : "Not Found"}`
  );
  console.log(
    `Auth Key Length: ${
      weddingConfig.messaging.msg91.authKey?.length || 0
    } characters`
  );
  console.log(
    `Auth Key Preview: ${weddingConfig.messaging.msg91.authKey?.substring(
      0,
      8
    )}...`
  );
  ...
  try {
    ...
    if (balance) {
      ...
      console.log(
        "⚠️ Could not fetch balance (but this might still work for sending)"
      );
    }
  } catch (error) {
